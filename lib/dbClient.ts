...
function normalizeArticle(record: any) {
  ...
  return {
    ...
    // include any other fields as-is for forward-compat
    ...record,
  }
}
...
export async function getArticles(filters?: { category?: string; search?: string; status?: string }) {
  if (!hasSupabase() || !supabase) throw new Error("Supabase not configured")
  try {
...
export async function getActiveJobs(filters?: { title?: string; location?: string }) {
  if (!hasSupabase() || !supabase) throw new Error("Supabase not configured")
  try {
    let query: any = supabase
      .from("jobs")
    ...
    const { data, error } = await query
    if (error) throw error
    return Array.isArray(data) ? data.map(normalizeJob) : data
  } catch (err) {
    ...
    try {
      ...
      if (files && files.length > 0) {
        const fileUploads = await Promise.all(
          files.map(file => uploadSubmissionFile(file, data.id))
        );
        ...
        if (successfulUploads.length > 0) {
          ...
          const { data: updatedData, error: updateError } = await supabase
            .from("submissions")
            .update({
              image_url: imageUrls[0] || null, // Store first image
              video_url: videoUrls[0] || null, // Store first video
              files: successfulUploads // Store all file metadata
            })
            .eq("id", data.id)
            .select()
            .single();
          ...
          return normalizeSubmission(updatedData);
        }
      }
...
export async function createAdvertisement(ad: { title: string; description: string; imageUrl?: string; link?: string; position: string }) {
  if (hasSupabase() && supabase) {
    try {
      const dbAd = {
        title: ad.title,
...
export async function updateAdvertisement(id: string, updates: Partial<{ title: string; description: string; imageUrl: string; link: string; position: string; isActive: boolean }>) {
  if (hasSupabase() && supabase) {
    try {
      const dbUpdates = {
        ...(updates.title && { title: updates.title }),
