...
import { Share2, ArrowLeft, Copy, Check, Facebook, Twitter, Linkedin, Mail } from "lucide-react"
...
export default function ArticlePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const response = await apiClient.get(`/api/articles?id=${params.id}&incrementViews=true`)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <section className="max-w-4xl mx-auto px-4 py-12 w-full">
        {/* Media Carousel - Build media array from all available sources */}
        {(() => {
          const allMedia: MediaItem[] = []
          
          // Use media array if it exists and has items
          if (article.media && article.media.length > 0) {
            allMedia.push(...article.media)
          } else {
            // Fallback: build media array from legacy fields
            if (article.imageUrl) {
              allMedia.push({ url: article.imageUrl, type: 'image' })
            }
            if (article.videoUrl) {
              allMedia.push({ url: article.videoUrl, type: 'video' })
            }
          }
          
          return allMedia.length > 0 ? (
            <div className="mb-8">
              <MediaCarousel media={allMedia} autoSlide={true} interval={5000} />
            </div>
          ) : null
        })()}
